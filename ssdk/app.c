...
#include "sl_rail_util_ant_div.h" //needed to check for antdiv prior to applying tweaks

//#define RAW_PACKET_DEBUG 1 //print length and RSSI of each packet when received

/* MG24 radio register defines for tweaking - Kris Young (kris.young@silabs.com) */
#define MODEM_ADQUAL8_ADBAMODE_ADDR      (0xB80142A8UL)
#define MODEM_ADQUAL8_ADBAMODE_REG       ((volatile uint32_t*) MODEM_ADQUAL8_ADBAMODE_ADDR)
#define MODEM_ADQUAL8_ADBAMODE_ANTDIV_MODE_MASK 0x300000 //bits 21-20
#define MODEM_ADQUAL8_ADBAMODE_ANTDIV_MODE_VAL 3U //ANTDIV MODE = RSSI only
#define MODEM_ADQUAL8_ADBAMODE_ANTDIV_MODE_SHIFT 20U //shift left 20

#define AGC_ANTDIV_DEBOUNCECNTTHD_ADDR      (0xB800C0D0UL)
#define AGC_ANTDIV_DEBOUNCECNTTHD_REG       ((volatile uint32_t*) AGC_ANTDIV_DEBOUNCECNTTHD_ADDR)
#define AGC_ANTDIV_DEBOUNCECNTTHD_GAIN_RESTORE_DEBOUNCE_MASK 0x1FC //bits 8-2
#define AGC_ANTDIV_DEBOUNCECNTTHD_GAIN_RESTORE_DEBOUNCE_VAL 80U //gain restore debounce = 80
#define AGC_ANTDIV_DEBOUNCECNTTHD_GAIN_RESTORE_DEBOUNCE_SHIFT 2U //shift left 2

#define AGC_CTRL1_RSSIPERIOD_ADDR      (0xB800C024UL)
#define AGC_CTRL1_RSSIPERIOD_REG       ((volatile uint32_t*) AGC_CTRL1_RSSIPERIOD_ADDR)
#define AGC_CTRL1_RSSIPERIOD_RSSI_MEAS_PERIOD_MASK 0xF00 //bits 11-8
#define AGC_CTRL1_RSSIPERIOD_RSSI_MEAS_PERIOD_VAL 3U //RSSI meas period 2^3 = 8
#define AGC_CTRL1_RSSIPERIOD_RSSI_MEAS_PERIOD_SHIFT 8U //shift left 8
/* ************************ */
...
static void helper_apply_antdiv_adjustment(void) {
  ...
  if (SL_RAIL_UTIL_ANTENNA_MODE_DIVERSITY == antennaMode) {
    ...
    // Adjust modem registers for consistent antenna selection behavior at high
    // signal levels. kris.young@#silabs.com
    sl_zigbee_app_debug_print("Applying antdiv reg adjustments in antenna diversity mode\r\n");
    sl_zigbee_app_debug_print("before antdiv reg adjustment:\r\n0x%x: 0x%x\r\n0x%x: 0x%x\r\n0x%x: 0x%x\r\n",
                              MODEM_ADQUAL8_ADBAMODE_ADDR, *MODEM_ADQUAL8_ADBAMODE_REG,
                              AGC_ANTDIV_DEBOUNCECNTTHD_ADDR, *AGC_ANTDIV_DEBOUNCECNTTHD_REG,
                              AGC_CTRL1_RSSIPERIOD_ADDR, *AGC_CTRL1_RSSIPERIOD_REG);

    // Write adjustment registers
    BUS_RegMaskedWrite(MODEM_ADQUAL8_ADBAMODE_REG,
                       MODEM_ADQUAL8_ADBAMODE_ANTDIV_MODE_MASK,
                       MODEM_ADQUAL8_ADBAMODE_ANTDIV_MODE_VAL << MODEM_ADQUAL8_ADBAMODE_ANTDIV_MODE_SHIFT);
    BUS_RegMaskedWrite(AGC_ANTDIV_DEBOUNCECNTTHD_REG,
                       AGC_ANTDIV_DEBOUNCECNTTHD_GAIN_RESTORE_DEBOUNCE_MASK,
                       AGC_ANTDIV_DEBOUNCECNTTHD_GAIN_RESTORE_DEBOUNCE_VAL << AGC_ANTDIV_DEBOUNCECNTTHD_GAIN_RESTORE_DEBOUNCE_SHIFT);
    BUS_RegMaskedWrite(AGC_CTRL1_RSSIPERIOD_REG,
                       AGC_CTRL1_RSSIPERIOD_RSSI_MEAS_PERIOD_MASK,
                       AGC_CTRL1_RSSIPERIOD_RSSI_MEAS_PERIOD_VAL << AGC_CTRL1_RSSIPERIOD_RSSI_MEAS_PERIOD_SHIFT);

    sl_zigbee_app_debug_print("after antdiv reg adjustment:\r\n0x%x: 0x%x\r\n0x%x: 0x%x\r\n0x%x: 0x%x\r\n",
                               MODEM_ADQUAL8_ADBAMODE_ADDR, *MODEM_ADQUAL8_ADBAMODE_REG,
                               AGC_ANTDIV_DEBOUNCECNTTHD_ADDR, *AGC_ANTDIV_DEBOUNCECNTTHD_REG,
                               AGC_CTRL1_RSSIPERIOD_ADDR, *AGC_CTRL1_RSSIPERIOD_REG);
  } else {
      sl_zigbee_app_debug_print("Not in RX antdiv mode, antdiv reg adjustment not applied!\r\n");
  }
...
static void send_ping_event_handler(sl_zigbee_af_event_t *event)
{
  ...
  sl_zigbee_app_debug_println("ping (st 0x%X)", status);
}
...
static void send_ack_event_handler(sl_zigbee_af_event_t *event)
{
  ...
  status = mfglibSendPacket(txCmdBuff, 0);
  sl_zigbee_app_debug_println("ack (st 0x%X)", status);
}
...
static void send_get_channel_response_event_handler(sl_zigbee_af_event_t *event)
{
  ...
  status = mfglibSendPacket(txCmdBuff, 0);
  sl_zigbee_app_debug_println("get ch resp (st 0x%X)", status);
}
...
static void send_set_power_event_handler(sl_zigbee_af_event_t *event)
{
  ...
  status = mfglibSendPacket(txCmdBuff, 0);
  sl_zigbee_app_debug_println("set pow (st 0x%X)", status);
}
...
static void send_get_power_event_handler(sl_zigbee_af_event_t *event)
{
  ...
  status = mfglibSendPacket(txCmdBuff, 0);
  sl_zigbee_app_debug_println("get pow (st 0x%X)", status);
}
...
static void send_get_power_response_event_handler(sl_zigbee_af_event_t *event)
{
  ...
  status = mfglibSendPacket(txCmdBuff, 0);
  sl_zigbee_app_debug_println("get pow resp (st 0x%X)", status);
}
...
static void send_stream_event_handler(sl_zigbee_af_event_t *event)
{
  ...
  status = mfglibSendPacket(txCmdBuff, 0);
  sl_zigbee_app_debug_println("stream (st 0x%X)", status);
}
...
static void send_start_rx_test_event_handler(sl_zigbee_af_event_t *event)
{
  ...
  status = mfglibSendPacket(txCmdBuff, 0);
  sl_zigbee_app_debug_println("start rx test (st 0x%X)", status);
}
...
static void send_end_rx_test_event_handler(sl_zigbee_af_event_t *event)
{
  ...
  status = mfglibSendPacket(txCmdBuff, 0);
  sl_zigbee_app_debug_println("end rx test (st 0x%X)", status);
}
...
static void send_report_event_handler(sl_zigbee_af_event_t *event)
{
  ...
  status = mfglibSendPacket(txCmdBuff, 0);
  sl_zigbee_app_debug_println("report (st 0x%X)", status);
...
static void send_reboot_event_handler(sl_zigbee_af_event_t *event)
{
  ...
  status = mfglibSendPacket(txCmdBuff, 0);
  sl_zigbee_app_debug_println("reboot (st 0x%X)", status);
}
...
static void send_get_reboot_byte_response_event_handler(sl_zigbee_af_event_t *event)
{
  ...
  status = mfglibSendPacket(txCmdBuff, 0);
  sl_zigbee_app_debug_println("get reboot byte (st 0x%X)", status);
}
...
static void send_get_hardware_version_event_handler(sl_zigbee_af_event_t *event)
{
  ...
  status = mfglibSendPacket(txCmdBuff, 0);
  sl_zigbee_app_debug_println("get hw ver (st 0x%X)", status);
}
...
static void send_get_hardware_version_response_event_handler(sl_zigbee_af_event_t *event)
{
  ...
  status = mfglibSendPacket(txCmdBuff, 0);
  sl_zigbee_app_debug_println("get hw ver resp (st 0x%X)", status);
}
...
static void send_get_software_version_event_handler(sl_zigbee_af_event_t *event)
{
  ...
  status = mfglibSendPacket(txCmdBuff, 0);
  sl_zigbee_app_debug_println("get sw ver (st 0x%X)", status);
}
...
static void send_get_software_version_response_event_handler(sl_zigbee_af_event_t *event)
{
  ...
  status = mfglibSendPacket(txCmdBuff, 0);
  sl_zigbee_app_debug_println("get sw ver resp (st 0x%X)", status);
}
...
static void stream_timer_event_handler(sl_zigbee_af_event_t *event)
{
  ...
  sl_zigbee_app_debug_println("stop stream (st 0x%X)", status);
}
...
static void custom_2_event_handler(sl_zigbee_af_event_t *event)
{
  ...
  if (status != SL_STATUS_OK) {
    ...
    sl_zigbee_app_debug_println("mfglib TX failure, status=0x%X", status);
    return;
  }
...
static void helper_app_init(void)
{
  ...
  sl_zigbee_app_debug_println("INIT: mfglib start (st 0x%X)", status);
...
static void helper_local_channel_with_power(uint8_t channel)
{
  ...
  sl_zigbee_app_debug_println("ch %x (st 0x%X)", channel, status);

  status = mfglibSetPower(SL_SRFT_TX_POWER_MODE, txPower);
  sl_zigbee_app_debug_println("pow %d (st 0x%X)", txPower, status);
}
...
static void helper_send_set_channel(void)
{
  ...
  status = mfglibSendPacket(txCmdBuff, 0);
  sl_zigbee_app_debug_println("set channel (st 0x%X)", status);
}
...
static void helper_set_power(void)
{
  ...
  sl_zigbee_app_debug_println("pow (st 0x%X)", status);
}
...
void sli_srft_cli_customSilabsAntDivAdjustment(sl_cli_command_arg_t *arguments);
...
static const sl_cli_command_info_t srft_cli_cmd_customSilabsAntDivAdjustment =
  SL_CLI_COMMAND(sli_srft_cli_customSilabsAntDivAdjustment,
                 "",
                 SL_CLI_UNIT_SEPARATOR,
{
  SL_CLI_ARG_END,
...
static const sl_cli_command_entry_t srft_cli_custom_silabs_command_table[] = {
  ...
  {"applyAntDivAdjust", &srft_cli_cmd_customSilabsAntDivAdjustment, false},
  { NULL, NULL, false },
};
...
/* CLI function for manually applying RX antdiv register adjustments */
void sli_srft_cli_customSilabsAntDivAdjustment(sl_cli_command_arg_t *arguments)
{
  helper_apply_antdiv_adjustment();
}
...
void app_init(void)
{
  // Apply antdiv adjustment automatically if defaulting to RX antdiv mode
  helper_apply_antdiv_adjustment();
...
static void mfglibRxHandler(uint8_t *packet, uint8_t linkQuality, int8_t rssi)
{
  ...
  if ((packet[1] == BYTE_SIGNATURE_FIRST) && (packet[2] == BYTE_SIGNATURE_SECOND) && (packet[3] == BYTE_TYPE_COMMAND)) {
    ...
    sl_zigbee_app_debug_print("len=%d, rssi=%d\r\n", packet[0], rssi);
#endif
  }
}
